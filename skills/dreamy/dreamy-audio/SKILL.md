...
Route audio behavior through verified package capability and keep audio feedback separate from gameplay authority.
...
Audio cue/config -> request source -> audio service/mixer -> playback handle -> lifetime/stop/fade -> settings persistence.
...
- Gameplay event may request audio; audio does not own gameplay result.
- Music/crossfade is app or scene owner; one-shot SFX may be local if dependency is passed.
- Volume/mute settings persist in Datasave/settings owner.
- Localized VO/subtitle keys require localization owner.
...
- Centralize mixer groups and volume settings.
- Pool frequent audio sources only when profiling or platform cost justifies it.

## Anti-patterns

- Animation/audio callback grants reward.
- Every button resolves global audio service.
- Ignoring pause/resume and app focus.
