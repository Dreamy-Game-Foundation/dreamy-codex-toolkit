...
Protect static tuning/catalog data and prevent config/save/runtime ownership drift.
...
Schema owner -> stable IDs -> authoring source -> validation -> runtime read-only service -> fallback/error policy.
...
- Validate duplicate IDs, missing references, required fields, and key stability.
- Keep runtime consumers read-only and centralized through a service/repository.

## Anti-patterns

- Mutating config as player state.
- Loading/parsing JSON independently in every panel.
- Silent defaults that hide missing production config.
