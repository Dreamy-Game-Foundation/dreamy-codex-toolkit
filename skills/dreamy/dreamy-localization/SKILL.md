...
Keep text keys stable and UI locale updates deterministic across panels, config, and release builds.
...
Stable key -> table/source -> locale store -> formatter/plural policy -> UI binding -> fallback/missing-key diagnostics.
...
- Static localized metadata belongs in localization/config tables.
- User locale preference persists in settings/save owner.
- Runtime UI must rebind on locale change.
- Missing key policy must be visible before release.
...
- Validate locale coverage and TMP/font assets.

## Anti-patterns

- Hardcoded text in reusable UI.
- Changing keys without migration.
- Silent fallback hiding missing production strings.
