...
Keep views focused on render/intent while presenters/services own business decisions, persistence, SDK, and navigation policy.
...
Screen/popup/overlay -> panel lifecycle -> bind state -> render -> collect intent -> presenter/service -> cleanup/cache/back.
...
- Presenter/controller/service owns economy, save, SDK, config parsing, and navigation policy.
- Panel reopen requires subscription, async, and tween cleanup.
- Tab state is runtime UI state unless explicitly persisted.
...
- ShopPanel edits coins.
- RewardButton writes save JSON.
- Panel parses shop catalog directly.
- Stacked tweens after repeated Show calls.
