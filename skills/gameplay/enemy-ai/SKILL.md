---
name: enemy-ai
description: Implement or review Unity gameplay sensing, target selection, decision state, actions, navigation, cooldown, fallback, death, disable, and pooling.
---
...
Guide sensing, target selection, decision state, actions, navigation, cooldown, fallback, death, disable, and pooling with explicit config/save/runtime ownership, lifecycle cleanup, feedback boundaries, and tests.
...
Sense -> TargetSelection -> DecisionState -> Action -> Navigation/CombatRequest -> Cooldown -> Fallback -> Death/DisableCleanup.
...
- AI state has one owner.
- Target lost/dead transitions intentionally.
- Pooled enemy resets nav, target, cooldowns, events.
...
- AI state scattered across Animator/NavMesh/booleans.
- Dead enemy still subscribed to events.
