---
name: game-state
description: Implement or review Unity gameplay match/session states, pause/resume, win/loss, restart, loading boundaries, and state transition guards.
---
...
Guide match/session states, pause/resume, win/loss, restart, loading boundaries, and state transition guards with explicit config/save/runtime ownership, lifecycle cleanup, feedback boundaries, and tests.
...
StateDefinition -> CurrentState -> Guard -> Exit -> Enter -> Event/Feedback.
...
- Use explicit state for mutually exclusive phases.
- Pause owner stops timers/input consistently.
...
- Boolean soup for match state.
- Timers keep running during pause.
