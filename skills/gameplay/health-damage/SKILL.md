---
name: health-damage
description: Implement or review Unity gameplay health, shields, damage requests, mitigation, healing, death idempotency, invulnerability, and combat feedback.
---
...
Guide health, shields, damage requests, mitigation, healing, death idempotency, invulnerability, and combat feedback with explicit config/save/runtime ownership, lifecycle cleanup, feedback boundaries, and tests.
...
DamageRequest -> Validation -> Mitigation -> HealthDelta -> DeathGuard -> Feedback.
...
- Death transition is idempotent.
- Damage calculation happens before feedback.
- Runtime owns current HP; config owns max/tuning.
