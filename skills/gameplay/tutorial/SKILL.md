---
name: tutorial
description: Implement or review Unity gameplay tutorial triggers, gates, step state, highlight/prompt UI, input blocking, persistence, and analytics.
---
...
Guide tutorial triggers, gates, step state, highlight/prompt UI, input blocking, persistence, and analytics with explicit config/save/runtime ownership, lifecycle cleanup, feedback boundaries, and tests.
...
TutorialDefinition -> StepState -> Trigger -> Gate/InputMask -> Prompt -> CompletionPersist.
...
- Gate only the relevant system/input.
- Completed steps persist; transient prompt state does not.
...
- Tutorial globally owns all systems.
- Hardcoded text or unlocalized prompts.
