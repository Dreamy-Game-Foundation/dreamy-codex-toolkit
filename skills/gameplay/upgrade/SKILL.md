---
name: upgrade
description: Implement or review Unity gameplay upgrade definitions, costs, requirements, unit/item level, cards, transactions, persistence, and UI intent.
---
...
Guide upgrade definitions, costs, requirements, unit/item level, cards, transactions, persistence, and UI intent with explicit config/save/runtime ownership, lifecycle cleanup, feedback boundaries, and tests.
...
UpgradeDefinition -> Eligibility -> Cost -> Transaction -> Level/StatGrant -> Persist -> UIRefresh.
...
- Costs/curves in DataConfig; player level/materials in Datasave.
- Upgrade transaction is atomic.
...
- Level stored in config.
- UI card deducts currency.
