...
description: Handle Android build/release: Gradle, AGP, JDK, SDK/NDK, AAB, IL2CPP, ARM64, manifest, R8/Proguard, signing, billing, and target SDK with evidence-first diagnosis, release impact, and platform-specific verification.
---
...
Guide Android build/release: Gradle, AGP, JDK, SDK/NDK, AAB, IL2CPP, ARM64, manifest, R8/Proguard, signing, billing, and target SDK without treating desktop compile or static checks as release proof.
...
UnityBuildSettings -> AndroidPlayerSettings -> Gradle/ManifestMerge -> R8 -> AAB/APK -> Signing -> StoreValidation.
...
- Classify compile vs Gradle vs R8 vs manifest vs signing failure.
- Do not mass-upgrade packages to try a fix.
- Signing/bundle ID/target SDK changes name release impact.
...
- Change keystore casually.
- Ignore merged manifest.
