...
description: Handle mobile production budgets for frame time, memory, thermal, battery, safe area, input, asset size, loading, and device validation with evidence-first diagnosis, release impact, and platform-specific verification.
---
...
Guide mobile production budgets for frame time, memory, thermal, battery, safe area, input, asset size, loading, and device validation without treating desktop compile or static checks as release proof.
...
TargetDeviceTier -> Budget -> FeatureRisk -> Profile/Build/Test -> Evidence -> ResidualRisk.
...
- UI needs safe-area/aspect/input verification.
- Asset size/compression affects build and memory.
...
- Pool everything without profile.
