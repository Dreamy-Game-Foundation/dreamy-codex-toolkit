---
name: store-release-readiness
description: Handle store release readiness: versions, build numbers, signing, symbols, privacy, IAP, ads, analytics, crash SDK, smoke tests, rollback, and changelog with evidence-first diagnosis, release impact, and platform-specific verification.
---
...
Guide store release readiness: versions, build numbers, signing, symbols, privacy, IAP, ads, analytics, crash SDK, smoke tests, rollback, and changelog without treating desktop compile or static checks as release proof.
...
ReleaseCandidate -> Version/Build -> StorePolicy -> SDKPrivacy -> SmokeTest -> RollbackPlan -> Approval.
...
- No release without verification evidence or explicit waiver.
- IAP/ads/privacy/crash SDK gates are release-sensitive.
...
- Ship with unknown build status.
- Skip privacy/store metadata.
