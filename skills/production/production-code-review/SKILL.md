...
description: Perform production code review for correctness, serialization, lifecycle, package boundaries, data ownership, security/privacy, tests, and release risk with findings-first output, evidence gates, and residual risk tracking.
---
...
Guide production code review for correctness, serialization, lifecycle, package boundaries, data ownership, security/privacy, tests, and release risk for shipping-quality changes.
...
Diff -> CorrectnessPass -> ArchitecturePass -> Severity -> Finding -> VerificationGap.
...
- Review behavior, lifecycle, serialization, async, pooling, asset lifetime, package direction.
- Style-only issues only when maintenance impact matters.
...
- Start with long summary.
- Approve without inspecting diff/tests.
