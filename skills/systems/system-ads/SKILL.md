---
name: system-ads
description: Implement or review mobile game system behavior for ads initialization, load, availability, placements, cooldown, frequency cap, rewarded callbacks, consent, persistence, and analytics.
---
...
Guide ads initialization, load, availability, placements, cooldown, frequency cap, rewarded callbacks, consent, persistence, and analytics with transaction safety, persistence, UI boundaries, lifecycle cleanup, and analytics ordering.
...
Initialize -> Load -> Availability -> ShowRequest -> SDKShow -> VerifiedRewardCallback -> IdempotentGrant -> Persist -> Analytics.
...
- Grant rewarded ads only on verified reward callback.
- Placement/cooldown/frequency cap owned by service.
- Consent gates SDK behavior.
...
- Read `references/rewarded-flow.md` when the task touches this skill's deeper rewarded flow behavior.
