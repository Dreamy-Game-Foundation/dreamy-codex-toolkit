...
description: Implement or review mobile game system behavior for analytics taxonomy, event ownership, PII safety, vendor abstraction, ordering, batching, consent, and failure behavior.
---
...
Guide analytics taxonomy, event ownership, PII safety, vendor abstraction, ordering, batching, consent, and failure behavior with transaction safety, persistence, UI boundaries, lifecycle cleanup, and analytics ordering.
...
DomainEvent -> AnalyticsEventMap -> Consent/PIIFilter -> AnalyticsService -> VendorSDK -> Retry/DropPolicy.
...
- Gameplay calls analytics service, not vendor SDK.
- Remove PII and secrets.
- Success events after committed result.
