---
name: system-daily-reward
description: Implement or review mobile game system behavior for daily rewards, time source, claim windows, streaks, timezone, offline clock, duplicate claim, reset, and persistence.
---
...
Guide daily rewards, time source, claim windows, streaks, timezone, offline clock, duplicate claim, reset, and persistence with transaction safety, persistence, UI boundaries, lifecycle cleanup, and analytics ordering.
...
CalendarPolicy -> TimeSource -> Eligibility -> ClaimTransaction -> Persist -> Streak/Reset -> UIRefresh.
...
- Declare server/device/offline time authority.
- Claims are idempotent and persisted.
...
- Device time silently authoritative.
- UI button writes claim state.
