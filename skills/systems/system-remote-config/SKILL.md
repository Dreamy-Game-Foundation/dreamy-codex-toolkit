...
description: Implement or review mobile game system behavior for remote config defaults, fetch, cache, fallback, parsing, activation, versioning, and cold-start behavior.
---
...
Guide remote config defaults, fetch, cache, fallback, parsing, activation, versioning, and cold-start behavior with transaction safety, persistence, UI boundaries, lifecycle cleanup, and analytics ordering.
...
LocalDefaults -> Fetch -> Validate/Parse -> Cache -> Activate -> ConsumerService -> Fallback.
...
- Block game forever on fetch failure.
- Panel parses remote config directly.
