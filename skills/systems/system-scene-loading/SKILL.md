---
name: system-scene-loading
description: Implement or review mobile game system behavior for scene loading, bootstrap order, additive scenes, progress UI, cancellation, asset lifetime, and transition cleanup.
---

# System Scene Loading

## Purpose

Guide scene loading, bootstrap order, additive scenes, progress UI, cancellation, asset lifetime, and transition cleanup with transaction safety, persistence, UI boundaries, lifecycle cleanup, and analytics ordering.
...
LoadRequest -> Bootstrap/SceneOwner -> AsyncLoad -> Activation -> DependencyReady -> Transition -> Unload/Cleanup.
...
- Bootstrap owns global services.
- Scene loader owns load/unload lifecycle.
- Progress UI does not own activation policy.
...
- Leaf UI starts raw scene load.
- Old scene assets released too early.
