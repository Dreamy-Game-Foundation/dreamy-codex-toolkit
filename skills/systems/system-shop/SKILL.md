---
name: system-shop
description: Implement or review mobile game system behavior for shop offers, eligibility, price, purchase transactions, reward grants, persistence, UI refresh, and analytics.
---
...
Guide shop offers, eligibility, price, purchase transactions, reward grants, persistence, UI refresh, and analytics with transaction safety, persistence, UI boundaries, lifecycle cleanup, and analytics ordering.
...
OfferDefinition -> ShopService -> Eligibility -> Price -> Transaction -> Grant -> Persist -> UIRefresh -> Analytics.
...
- Offer definitions in DataConfig.
- Currency/redeemed state in Datasave.
- UI sends purchase intent only.
- Availability is checked in service, not just UI.
...
- Hidden offer still purchasable.
- Currency deducted without grant.
...
- Read `references/transaction-flow.md` when the task touches this skill's deeper transaction flow behavior.
