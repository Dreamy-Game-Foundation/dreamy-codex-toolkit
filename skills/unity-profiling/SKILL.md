...
description: Use Unity profiling workflow for CPU, GPU, GC, UI rebuild, physics, loading, battery, thermal, and before/after evidence with Unity-safe ownership, serialized asset safety, and verification evidence.
---
...
Guide Unity profiling workflow for CPU, GPU, GC, UI rebuild, physics, loading, battery, thermal, and before/after evidence for Unity projects.
...
Budget -> Repro -> ProfilerCapture -> BottleneckClass -> FocusedFix -> AfterCapture.
...
- Define target budget and representative hardware.
- One focused optimization at a time.
