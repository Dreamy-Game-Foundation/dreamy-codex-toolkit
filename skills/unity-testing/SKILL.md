---
name: unity-testing
description: Use Unity test selection for pure C#, EditMode, PlayMode, editor integration, build validation, device checks, fixtures, and evidence with Unity-safe ownership, serialized asset safety, and verification evidence.
---
...
Guide Unity test selection for pure C#, EditMode, PlayMode, editor integration, build validation, device checks, fixtures, and evidence for Unity projects.
...
Risk -> TestLayer -> Fixture -> Command -> Evidence -> Gap.
...
- Lowest layer that proves behavior.
- Serialization/prefab uses EditMode/fixture.
- Lifecycle uses PlayMode.
...
- Broad PlayMode for pure logic.
- Done without not-run reason.
...
- Use `references/proof-selection-matrix.md` to pick the lowest test layer that proves the risk.
